...
router.post('/sorting-units', csrfProtection, async (req, res) => {
  try {
    ...
    const municipality = typeof req.body.municipality === 'string' ? req.body.municipality.trim() : null;
    const description = typeof req.body.description === 'string' ? req.body.description.trim() : null;

    if (!name) {
      return res.status(400).json({ error: 'Lajitteluyksikön nimi on pakollinen.' });
    }
...
router.post('/fish', csrfProtection, async (req, res) => {
  try {
    const finnishName = typeof req.body.finnishName === 'string' ? req.body.finnishName.trim() : '';
    ...
    if (error.name === 'SequelizeUniqueConstraintError') {
      return res.status(409).json({ error: 'Talla nimella oleva kala on jo olemassa.' });
    }
  ...
  try {
    ...
    if (error.name === 'SequelizeForeignKeyConstraintError') {
      return res.status(409).json({
        error: 'Kalaa ei voi poistaa, koska siihen viittaa olemassa olevia havaintoja.'
      });
    }
...
router.post('/change-password', csrfProtection, async (req, res) => {
  ...
  const renderPage = (values) => res.render('admin/changePassword', {
    title: 'Fish AI - Vaihda salasana',
    adminLayout: true,
    activeTab: 'change-password',
    csrfToken: req.csrfToken(),
  ...
  if (newPassword.length < 8) {
    return renderPage({ error: 'Uuden salasanan on oltava vahintaan 8 merkkia' });
  }
